...
class Tiene {
    ...
    public function listaEspecifica() {
        $db = new Conexion();
        $sql = $db->query("SELECT * FROM tiene WHERE ci_contacto = $this->ci_contacto");
        return ($sql);
    }
    ...
    public function elimina() {
        $db = new Conexion();
        $sql = $db->query("DELETE FROM tiene WHERE ci_contacto = '$this->ci_contacto'");
        return ($sql);
    }
    ...
    public function modifica() {
        $db = new Conexion();
        $sql = $db->prepare("UPDATE tiene SET parentesco = ? WHERE ci_contacto = ?");
        $sql->bind_param("si", $this->parentesco, $this->ci_contacto);
        $result = $sql->execute();
